...
GUI mode (for students):
  python salmon_qc_gui_cli.py

CLI mode (reproducible / pipeline):
...
Inputs:
  - Up to 5 Salmon files (TSV/tabular) with columns: Name, TPM
  - FASTA with headers containing [GeneID=12345] (same logic as your SalmonGeneMapper)
  - Gene table (optional): colunas "NCBI GeneID", "Symbol", "Description"
...
def build_map_from_fasta(fasta_path: Path) -> Tuple[Dict[str, str], Dict[str, str]]:
    """Returns: tx_to_gene, tx_to_geneid"""
    ...
    if not tx_to_geneid:
        raise ValueError(
            "Could not extract GeneID from the FASTA headers.\n"
            "It must contain something like: >TRANSCRIPTO GENE ... [GeneID=12345]"
        )
...
def read_salmon(path: Path) -> pd.DataFrame:
    """Reads a Salmon TSV/tabular file. Expects Name and TPM."""
    df = pd.read_csv(path, sep="\t")

    # some exports change the name of the first column
...
def salmon_to_gene_tpm(quant_path: Path, sample_label: str,
    ...
    """
    Converts one quantification file to gene level (summed TPM).
    Returns: GeneID, GeneName, TPM_<label>
    """
...
def merge_gene_tables(per_sample: List[pd.DataFrame]) -> pd.DataFrame:
    """Merges by GeneID and resolves GeneName."""
    wide = per_sample[0]
...
# -----------------------------
# PCA / Outlier / Discrepant genes
# -----------------------------
def detect_outlier_pca(scores: pd.DataFrame) -> Tuple[str, pd.Series]:
    """Outlier = largest mean distance in PC1-PC2 space."""
...
def compute_outlier_genes(expr_log: pd.DataFrame, outlier: str, top_n: int) -> pd.DataFrame:
    """
    expr_log: index=GeneID, columns=samples with log2(TPM+1)
...
def pca_plot(scores: pd.DataFrame, explained: np.ndarray, out_png: Path, outlier: str) -> None:
    ...
    for _, r in scores.iterrows():
        ...
        # Name appears once (including the outlier) — without "OUTLIER:" above it
...
def run_analysis(cfg: RunConfig, script_path: str) -> Dict[str, str]:
    if not (2 <= len(cfg.inputs) <= 5):
        raise ValueError("Select between 2 and 5 Salmon files.")
    ...
    report.append(f"Genes (after filtering): {expr.shape[0]}\n")
...
class App(tk.Tk):
    ...
    def _build(self):
        ...
        frm1 = tk.LabelFrame(self, text="1) Salmon files (2 to 5) [Name + TPM]")
        frm1.pack(fill="x", **pad)

        btn_sel = tk.Button(frm1, text="Select Salmon files...", command=self.pick_salmon_files)
        btn_sel.pack(anchor="w", padx=10, pady=6)

        self.files_box = tk.Text(frm1, height=6, width=100)
        self.files_box.pack(padx=10, pady=6)

        frm2 = tk.LabelFrame(self, text="2) Reference (FASTA) and gene table (optional)")
        frm2.pack(fill="x", **pad)

        btn_fa = tk.Button(frm2, text="Select FASTA (required)", command=self.pick_fasta)
        ...
        btn_gt = tk.Button(frm2, text="Select gene_table TSV (optional)", command=self.pick_gene_table)
        ...
        frm3 = tk.LabelFrame(self, text="3) Output and parameters")
        frm3.pack(fill="x", **pad)

        btn_out = tk.Button(frm3, text="Select output folder", command=self.pick_outdir)
        ...
        tk.Label(frm3, text="Top discrepant genes:").grid(row=1, column=0, padx=10, pady=6, sticky="w")
        ...
        tk.Label(frm3, text="min_TPM filter (>= in at least 2 samples; 0 = disabled):").grid(row=2, column=0, padx=10, pady=6, sticky="w")
        ...
        self.run_btn = tk.Button(frm4, text="RUN ANALYSIS", command=self.run_clicked, bg="#e6e6e6")
        self.run_btn.pack(side="left", padx=10, pady=10)

        self.status = tk.Label(frm4, text="Status: waiting for file selection.")
        self.status.pack(side="left", padx=10)

        frm5 = tk.LabelFrame(self, text="Generated outputs (reproducible)")
        frm5.pack(fill="both", expand=True, **pad)

        help_txt = (
            "O programa salva no outdir:\n"
            "- pca_PC1_PC2_gene.png (PCA; outlier in red)\n"
            "- corr_heatmap_gene.png (correlation)\n"
            "- top_outlier_genes_annotated.csv (most discrepant genes in the outlier)\n"
            "- outlier_report_gene.txt (outlier identity and metrics)\n"
    ...
    def pick_salmon_files(self):
        files = filedialog.askopenfilenames(
            title="Select 2 a 5 files Salmon (TSV/tabular)",
            filetypes=[("TSV/tabular", "*.tsv *.tabular *.txt *.*"), ("All files", "*.*")]
        )
        if not files:
            return
        if len(files) < 2 or len(files) > 5:
            messagebox.showerror("Error", "Select between 2 and 5 files.")
            return
        ...
        self.status.configure(text="Status: Salmon selected. Now select the FASTA and output folder.")

    def pick_fasta(self):
        f = filedialog.askopenfilename(
            title="Select the reference FASTA (headers containing [GeneID=...])",
    ...
    def pick_gene_table(self):
        f = filedialog.askopenfilename(
            title="Select gene_table TSV (optional)",
    ...
    def pick_outdir(self):
        d = filedialog.askdirectory(title="Select a output folder")
        if not d:
            return
        self.outdir = d
    ...
    def run_clicked(self):
        try:
            if not self.salmon_files or len(self.salmon_files) < 2:
                messagebox.showerror("Error", "Select 2 a 5 files Salmon.")
                return
            if not self.fasta_path:
                messagebox.showerror("Error", "Select o FASTA (required).")
                return
            if not self.outdir:
                messagebox.showerror("Error", "Select a output folder.")
                return
            ...
            self.status.configure(text=f"Status: completed. Outlier = {res['outlier']}")

        except Exception as e:
            messagebox.showerror("Error", str(e))
            self.status.configure(text="Status: error. See the message.")
...
def parse_args():
    ...
    runp = sub.add_parser("run", help="Run analysis via CLI (reproducible)")
    runp.add_argument("--inputs", nargs="+", required=True, help="2 a 5 files Salmon (Name, TPM)")
    runp.add_argument("--names", nargs="+", required=False, help="Sample names (if omitted, basename is used)")
    runp.add_argument("--fasta", required=True, help="FASTA with headers containing [GeneID=...]")
    runp.add_argument("--gene_table", required=False, help="TSV with NCBI GeneID / Symbol / Description (optional)")
    runp.add_argument("--outdir", required=True, help="Output folder")
    runp.add_argument("--top_outlier_genes", type=int, default=50, help="Top N discrepant genes in the outlier")
    runp.add_argument("--min_tpm", type=float, default=1.0, help="TPM filter (>= in >=2 samples); 0 disables it")
...
def main():
    ...
    if args.cmd == "run":
        ...
        if not (2 <= len(inputs) <= 5):
            raise SystemExit("Provide between 2 and 5 files in --inputs.")

        if args.names is None:
            names = [Path(p).stem for p in inputs]
        else:
            if len(args.names) != len(inputs):
                raise SystemExit("The number of --names entries must match the number of --inputs entries.")
            names = args.names
